...
class Post:
    ...
    async def create_post(self, preview_image: BinaryIO, scheme: List[List[Tuple[int, int]]],
        ...
        total_length = sum(count * length for row in scheme for count, length in row)
        post = {
            ...
            "comment": comment[:512],
            "author_id": author_id,
            "color_count": len({color for row in scheme for color, _ in row}),
            "length": total_length,
